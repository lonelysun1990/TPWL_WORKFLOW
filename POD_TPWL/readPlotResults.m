function [] = readPlotResults(caseDir, caseName, wellSchedule, targetSchedule, trainingSchedule)
% simply read all .mat file needed and plot
% this code will be more elegant and efficient later
iDir = [caseDir 'data/'];
oDir = [caseDir 'data/figure_output/'];
[timeRec, well] = readResults(iDir, targetSchedule, trainingSchedule);
% plotting part
plotToolKit(oDir, caseName, wellSchedule, timeRec, well);
end
...
function [] = plotToolKit(oDir, caseName, wellSchedule, timeRec, well)
...
figure_name = [oDir caseName '_schedule_' int2str(wellSchedule) '_well_' int2str(iWell)];
