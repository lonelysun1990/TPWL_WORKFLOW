...
[ctrlTraining] = scheduleConvert(ioDir, trainingSchedule(1), var.time, 'perm');
[ctrlTarget] = scheduleConvert(ioDir, targetSchedule, var.time, 'perm');
...
for iStep = 1 : size(var.time, 1) - 1
    ...
    ctrlReduced = var.U{iStep,1}*((ctrlTarget - ctrlTraining)./ctrlTraining);
    stateInc = JrInv \ (AccReduced + ctrlReduced);
%     stateInc = JrInv \ AccReduced;
